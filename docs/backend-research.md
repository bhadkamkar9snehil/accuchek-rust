...
| --- | --- | --- |
| `tidepool-org/uploader` (`lib/drivers/roche/accuChekUSB.js`) | Maintained Roche IEEE 11073/WebUSB state machine, MDS metadata parsing, transfer retry/error behavior, high/low sentinels | Primary maintained reference |
...
| `emogenet/accuchek` | Native C++/libusb implementation of the same meter protocol | Independent native transport implementation, originally informed by Tidepool |
...
GitHub forks/copies of `emogenet/accuchek` were not counted as independent evidence. Pump/CGM projects using the Accu-Chek name were also excluded because they do not exercise the Instant USB PHDC path.
...
- Unknown/nonzero status records remain visible in raw data but are excluded from headline numerical analysis until their semantics are understood.
...
Tidepool issue #1708 (opened 2025-11-22) documents that before-meal, after-meal and sleep flags are not currently surfaced by its Accu-Chek USB driver. Until those bits/fields are verified against captures or specification material, this project will store the raw status word and will not invent interpretations.
